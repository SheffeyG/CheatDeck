...
const expectSuccess = (result: LaunchOptionsEditResult): LaunchOptions => {
...
describe("LaunchOptions", () => {
  describe("reading and preservation", () => {
    ...
    it("preserves unknown parameters, whitespace, and quoting while patching one token", () => {
      ...
      const edited = expectSuccess(LaunchOptions.parse(source).setDxvkAsync(true));
    ...
    it("does not mutate the original instance", () => {
      ...
      const edited = expectSuccess(original.setLanguage("fr_FR.utf8"));
  ...
  describe("managed options", () => {
    it("sets and disables trainer options", () => {
      const enabled = expectSuccess(LaunchOptions.parse("").setTrainer("/home/deck/trainers/game.exe"));
      ...
      const disabled = expectSuccess(enabled.disableTrainer());
    ...
    it("sets and disables both language variables", () => {
      const enabled = expectSuccess(LaunchOptions.parse("%command%").setLanguage("de_DE.UTF-8"));
      ...
      const disabled = expectSuccess(enabled.disableLanguage());
    ...
    it("sets and disables a compatibility path", () => {
      ...
      const disabled = expectSuccess(enabled.disableCompatibilityPath());
    ...
    it("toggles DXVK, RADV, and Lossless Scaling", () => {
      ...
      options = expectSuccess(options.setDxvkAsync(true));
      options = expectSuccess(options.setRadvPerftest(true));
      options = expectSuccess(options.setLosslessScaling(true));
      expect(options.isDxvkAsyncEnabled).toBe(true);
      expect(options.isRadvPerftestEnabled).toBe(true);
      expect(options.isLosslessScalingEnabled).toBe(true);

      options = expectSuccess(options.setDxvkAsync(false));
      options = expectSuccess(options.setRadvPerftest(false));
      options = expectSuccess(options.setLosslessScaling(false));
    ...
    it("keeps Framegen patch and unpatch mutually exclusive", () => {
      let options = expectSuccess(LaunchOptions.parse("").setFramegenPatch(true));
      expect(options.isFramegenPatchEnabled).toBe(true);
      expect(options.isFramegenUnpatchEnabled).toBe(false);

      options = expectSuccess(options.setFramegenUnpatch(true));
      expect(options.isFramegenPatchEnabled).toBe(false);
      expect(options.isFramegenUnpatchEnabled).toBe(true);

      options = expectSuccess(options.setFramegenPatch(true));
      expect(options.isFramegenPatchEnabled).toBe(true);
      expect(options.isFramegenUnpatchEnabled).toBe(false);
    });
    ...
    it("removes duplicate managed entries and leaves unknown entries intact", () => {
      ...
      const enabled = expectSuccess(LaunchOptions.parse(source).setDxvkAsync(true));
    ...
    it("supports consecutive immutable edits", () => {
      const first = expectSuccess(LaunchOptions.parse("").setLanguage("en_US.UTF-8"));
      const second = expectSuccess(first.setDxvkAsync(true));
      const third = expectSuccess(second.setCompatibilityPath("/home/deck/prefix"));
  ...
  describe("custom options", () => {
    ...
    it("enables and disables each custom option type", () => {
      ...
      for (const option of customOptions) options = expectSuccess(options.setCustomOption(option, true));
      for (const option of customOptions) expect(options.isCustomOptionEnabled(option)).toBe(true);

      for (const option of customOptions) options = expectSuccess(options.setCustomOption(option, false));
    ...
    it("matches and deduplicates a quoted prefix command", () => {
      ...
      const enabled = expectSuccess(LaunchOptions.parse("").setCustomOption(option, true));
      const enabledAgain = expectSuccess(enabled.setCustomOption(option, true));
  ...
  describe("source-safe editing", () => {
    it("round-trips trainer and compatibility paths with shell-special characters", () => {
      ...
      let options = expectSuccess(LaunchOptions.parse("").setTrainer(path));
      options = expectSuccess(options.setCompatibilityPath(directory));
    ...
    it("removes duplicate prefix commands without leaving a separator", () => {
      ...
      const edited = expectSuccess(LaunchOptions.parse("cmd -- cmd -- cmd %command%").setCustomOption(option, true));
